...
{
	ShowClient=1, AddingClient = 2, DeleteClient = 3, UpdateClient = 4, FindClient = 5,Transactions=6,Exit=7
};
enum enMenueTransactionOptions
{
	eDeposit=1,eWithdraw,eTotalBalances,eMainMenue
};
void PreformTransactionsMenue(enMenueTransactionOptions);
...
void DepositScreen() {
	PrintHeaderScreen("Deposit Screen");
	string AccountNumber = ReadAccountNumber();
	vector<stClient>Vclients=LoadDataClientsFromFile(FileName);
	Deposit(AccountNumber,Vclients);
	SaveClientsInFile(FileName,Vclients);
}
double GetTotalBalance(vector<stClient>& Vclients) {
...
void TotalBalancesScreen() {
	...
	if (Vclients.size()==0){
		...
		cout << "\n______________________________________________________________________________________________\n\n";
		cout << "\t\t\t\t Total Balances = " << GetTotalBalance(Vclients)<<endl;
	}
}
void Withdraw(string AccountNumber, vector<stClient>& Vclients) {
	...
	if (FindDataClient(AccountNumber, Client)) {
		PrintClientCard(Client);
		double Amount = ReadWithdrawAmount(Client.AccountBalance);
		cout << "\nAre you sure you want preform this transaction? y/n ?   \n";
		cin >> answer;
		if (tolower(answer) == 'y') {
			for (stClient& C : Vclients) {
...
void WithdrawScreen() {
	PrintHeaderScreen("Withdraw Screen");
	string AccountNumber = ReadAccountNumber();
	vector<stClient>Vclients = LoadDataClientsFromFile(FileName);
...
void ShowTransactionsMenueScreen() {
	...
	cout << "\t[4] Main Menue.\n";
	cout << "=============================================\n";
	PreformTransactionsMenue((enMenueTransactionOptions)TrancsactionChoice());
}
void GoBackTransactionsMenue() {
	system("pause");
	ShowTransactionsMenueScreen();
}
void PreformTransactionsMenue(enMenueTransactionOptions MenueTransaction) {
	switch (MenueTransaction) {
	 case enMenueTransactionOptions::eDeposit:
		 system("cls");
		 DepositScreen();
		 GoBackTransactionsMenue();
		 break;
	 case enMenueTransactionOptions::eMainMenue:
		 system("cls");
		 ShowMainMenue();
		 break;
	 case enMenueTransactionOptions::eTotalBalances:
		 system("cls");
		 TotalBalancesScreen();
		 GoBackTransactionsMenue();
		 break;
	 case enMenueTransactionOptions::eWithdraw:
		 system("cls");
		 WithdrawScreen();
		 GoBackTransactionsMenue();
		 break;
